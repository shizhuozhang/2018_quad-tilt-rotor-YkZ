...
/**串口1初始化函数*/
	...
	//默认1起始位，8数据位，1停止位,无奇偶校验
		...
		MY_NVIC_Init(9,0,USART1_IRQn,4);//组4，最低优先级  
	...
	void USART1_IRQHandler(void)
	{
		...
		USART1->SR&=~(1<<5);//读完数据该标志位自动清零，没必要用这行代码手动清除标志位
	} 
#endif
...
/**串口2初始化函数*/
	...
	//默认1起始位，8数据位，1停止位,无奇偶校验
		...
		MY_NVIC_Init(6,0,USART2_IRQn,4);//组4，最低优先级 
...
u8 Ahrs_Flag1=1;
	...
	void USART2_IRQHandler(void)
	{
		...
		USART2->SR&=~(1<<5);//读完数据该标志位自动清零，没必要用这行代码手动清除标志位
	} 
#endif
...
//		USART3->CR1|=0X200C;  //1位停止,无校验位.
	//默认1起始位，8数据位，1停止位,无奇偶校验
		...
		MY_NVIC_Init(5,0,USART3_IRQn,4);//组4，
	...
	void USART3_IRQHandler(void)
	{
		...
		USART3->SR&=~(1<<5);////读完数据该标志位自动清零，没必要用这行代码手动清除标志位
	} 
#endif
...
/**串口4初始化函数*/
	...
	//默认1起始位，8数据位，1停止位,无奇偶校验
		...
		MY_NVIC_Init(4,0,USART4_IRQn,4);//组4，最低优先级 
	...
	//	UART4->SR&=~(1<<5);//读完数据该标志位自动清零，没必要用这行代码手动清除标志位
	} 
#endif
...
/**串口5初始化函数*/
	...
	//默认1起始位，8数据位，1停止位,无奇偶校验
		...
		MY_NVIC_Init(3,0,USART5_IRQn,4);//组4，最低优先级  
	...
	//	UART5->SR&=~(1<<5);//读完数据该标志位自动清零，没必要用这行代码手动清除标志位
	} 
#endif
...
/**串口6初始化函数*////未验证，应该没问题
	...
	//默认1起始位，8数据位，1停止位,无奇偶校验
		...
		MY_NVIC_Init(3,0,USART6_IRQn,4);//组4，最低优先级 
	...
	//	USART6->SR&=~(1<<5);//读完数据该标志位自动清零，没必要用这行代码手动清除标志位		
	} 
#endif
