...
//	DMA_Streamx->FCR=0X21;	//FIFO控制寄存器？？？？？？？？？？
	...
	MY_NVIC_Init(5,0,DMA1_Stream1_IRQn,4);//需要根据需要修改中断矢量号
...
void  DMA1_Stream1_IRQHandler()//数据流X0-3的传输完成中断服务函数
{
	...
	{	
		...
		if((SBUS_DATA[24]==0)&&(SBUS_data_Handler!=NULL))	//接收到数据，并且二值信号量有效
		{			
//			num1++;
			vTaskNotifyGiveFromISR(SBUS_data_Handler,&xHigherPriorityTaskWoken1);//释放二值信号量
...
//	DMA_Streamx->FCR=0X21;	//FIFO控制寄存器？？？？？？？？？？
	...
	MY_NVIC_Init(6,0,DMA1_Stream5_IRQn,4);//需要根据需要修改中断矢量号
...
{
	BaseType_t xHigherPriorityTaskWoken5;
	if(DMA1->HISR&(1<<11))//传输完成标志位，数据流4-7对应5，11，21，27
	{
		...
		if((0XA1==AHRS_DATA[1])&&(0XAA==AHRS_DATA[41])&&(AHRS_data_Handler!=NULL))		//再次判断帧标识字节和结尾校验	
		{							
//			num2++;
			vTaskNotifyGiveFromISR(AHRS_data_Handler,&xHigherPriorityTaskWoken5);//释放二值信号量
			portYIELD_FROM_ISR(xHigherPriorityTaskWoken5);//如果需要的话进行一次任务切换			
		}	
...
//	DMA_Streamx->CR|=1<<6;		//外设到存储器模式//0是从外设到存储器，1是存储器到外设，2是存储器到存储器
	DMA_Streamx->CR|=1<<4;		//传输完成中断使能	
	MY_NVIC_Init(12,0,DMA2_Stream0_IRQn,4);//需要根据需要修改中断矢量号	
