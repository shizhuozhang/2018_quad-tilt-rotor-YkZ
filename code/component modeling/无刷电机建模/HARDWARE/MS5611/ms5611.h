...
#define MS5611_CS(X)			(X==0)?GPIO_ResetBits(GPIOB,GPIO_Pin_12):GPIO_SetBits(GPIOB,GPIO_Pin_12) //MS5611片选信号

void SPI2_Init(void);
extern uint8_t MS5611_Write_Reg(uint8_t reg,uint8_t value);
extern uint8_t MS5611_Read_Reg(uint8_t reg);
extern uint8_t SPI2_ReadWriteByte(uint8_t TxData);//SPI总线读写一个字节
