...
//	RCC->APB2ENR|=1<<2;		//IO端口A的时钟使能
    ...
    TIM2->CCR1=1000;    	//初始装载值
	TIM2->CCR2=1000;    	//初始装载值
	TIM2->CCR3=1000;    	//初始装载值
	TIM2->CCR4=1000;    	//初始装载值
	//PWM输出模式1下，占空比的计算方法为CCR1/ARR*100%
	//PWM输出模式2下，占空比的计算方法为(1-CCR1/ARR)*100%
} 
#endif
...
{		 					 
    ...
    TIM3->CCR1=3000;    	//初始装载值
	TIM3->CCR2=3000;    	//初始装载值
	TIM3->CCR3=3000;    	//初始装载值
	TIM3->CCR4=3000;    	//初始装载值
	//PWM输出模式1下，占空比的计算方法为CCR1/ARR*100%
	//PWM输出模式2下，占空比的计算方法为(1-CCR1/ARR)*100%
} 
#endif
...
{		 					 
    ...
    TIM4->CCR1=1000;    	//初始装载值
	TIM4->CCR2=1000;    	//初始装载值
	TIM4->CCR3=1000;    	//初始装载值
	TIM4->CCR4=1000;    	//初始装载值
	//PWM输出模式1下，占空比的计算方法为CCR1/ARR*100%
	//PWM输出模式2下，占空比的计算方法为(1-CCR1/ARR)*100%
} 
#endif
...
{		 					 
	...
	TIM5->CCMR1|=0X0D<<3;  	//CH1输出,PWM1模式,预装载使能			 
	TIM5->CCMR1|=0X0D<<11; 	//CH2输出,PWM1模式,预装载使能
	TIM5->CCMR2|=0X0D<<3;  	//CH3输出,PWM1模式,预装载使能			 
	TIM5->CCMR2|=0X0D<<11; 	//CH4输出,PWM1模式,预装载使能
							
 	TIM5->CCER|=0X1111;   	//OC1,2,3,4 输出使能，输出极性为高电平有效	   

	TIM5->CR1=0x0080;   	//向上计数，自动重装载使能 
	TIM5->CR1|=0x01;    	//使能定时器5 
	
    TIM5->CCR1=1000;    	//初始装载值
	TIM5->CCR2=1000;    	//初始装载值
	TIM5->CCR3=1000;    	//初始装载值
	TIM5->CCR4=1000;    	//初始装载值
	//PWM输出模式1下，占空比的计算方法为CCR1/ARR*100%
	//PWM输出模式2下，占空比的计算方法为(1-CCR1/ARR)*100%
} 
#endif
...
{		 					 
	...
	TIM8->CCMR1|=0X0D<<3;  	//CH1输出,PWM1模式,预装载使能			 
	TIM8->CCMR1|=0X0D<<11; 	//CH2输出,PWM1模式,预装载使能
	TIM8->CCMR2|=0X0D<<3;  	//CH3输出,PWM1模式,预装载使能			 
	TIM8->CCMR2|=0X0D<<11; 	//CH4输出,PWM1模式,预装载使能
							
 	TIM8->CCER|=0X1111;   	//OC1,2,3,4 输出使能，输出极性为高电平有效	   
	TIM8->BDTR|=1<<15;   	//高级定时器特有的寄存器，MOE 主输出使能	   

	TIM8->CR1=0x0080;   	//向上计数，自动重装载使能 
	TIM8->CR1|=0x01;    	//使能定时器8 
	
    TIM8->CCR1=1000;    	//初始装载值
	TIM8->CCR2=1000;    	//初始装载值
	TIM8->CCR3=1000;    	//初始装载值
	TIM8->CCR4=1000;    	//初始装载值
	//PWM输出模式1下，占空比的计算方法为CCR1/ARR*100%
	//PWM输出模式2下，占空比的计算方法为(1-CCR1/ARR)*100%
} 
#endif
